...
a = TerrainGrid((SRTM4), (1,1), 0).arrayValues

min = 0
xMax = a.shape[1] - 1
yMax = a.shape[0] - 1

X = np.linspace(min, xMax, a.shape[1])
Y = np.linspace(min, yMax, a.shape[0])

x, y = np.meshgrid(X, Y)

f = interpolate.interp2d(x, y, a)
Xnew = np.linspace(min, xMax, a.shape[1] * 6)
Ynew = np.linspace(min, yMax, a.shape[0] * 6)

plt.imshow(f(Xnew, Ynew))
plt.show()
